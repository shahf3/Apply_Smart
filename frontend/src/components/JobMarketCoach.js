...
const ViewToggle = ({ viewMode, setViewMode }) => (
  <div className="relative w-64 mx-auto p-1 bg-gray-700/50 rounded-full flex items-center">
    <motion.div layout transition={{ type: "spring", stiffness: 300, damping: 30 }} className="absolute inset-0 p-1">
      <div
...
export default function JobMarketCoach() {
  ...
  const fetchNews = async () => {
    ...
    setError("");
    try {
      const { data } = await api.get("/api/news/job-news", {
        params: { ts: Date.now() },
      });
          ...
          return (
            <motion.article
              ...
              className={`cursor-pointer bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border-2 ${
                isSelected ? "border-indigo-500 ring-4 ring-indigo-500/20" : "border-transparent hover:border-gray-200 dark:hover:border-gray-700"
              }`}
              onClick={() => {
  ...
  return (
    <div className="min-h-screen">
      <motion.div className="text-center mb-10" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <motion.h1 className="text-4xl md:text-5xl font-bold text-gray-800 dark:text-white mb-4">
          Job Market <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">Intelligence</span>
        </motion.h1>
        <motion.p className="text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
