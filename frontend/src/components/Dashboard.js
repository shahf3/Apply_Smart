...
function Dashboard() {
  ...
  const loadRecentActivities = useCallback(async (uid) => {
    try {
  ...
  // 2) Use them in the effect and depend on them
  useEffect(() => {
    let cancelled = false;
    ...
    const initializeDashboard = async () => {
      try {
        ...
        if (storedId) {
          ...
          await Promise.all([
            loadDashboardStats(storedId),
            loadRecentActivities(storedId),
          ]);
        } else {
  ...
  }, [loadDashboardStats, loadRecentActivities, calculateProfileCompletion]);
